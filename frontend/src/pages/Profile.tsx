...
export default function Profile() {
  ...
  async function handleDeleteUser(u: UserRow) {
    ...
    try {
      ...
      setNotice(`Deleting user "${u.name}"...`);
      // call API deleteUser (api.deleteUser was added in src/api.ts)
      await (api.deleteUser ? api.deleteUser(u.id) : Promise.reject(new Error('deleteUser API not available')));
      ...
      setNotice(`User "${u.name}" deleted.`);
      // clear notice after a short duration
      setTimeout(() => setNotice(null), 3500);
    } catch (err: any) {
      // clear any transient notice
      setNotice(null);
      // If our requestJson set status/body, use those
      const status = err?.status;
      const body = err?.body;
      if (status === 409) {
        // Conflict — show specific message if server provided one
                    ...
                    <table className="min-w-full divide-y">
                      ...
                      <tbody className="bg-white divide-y">
                        {pageItems.length === 0 ? (
                          ...
                          <tr
                            key={r.id}
                            className={`hover:bg-slate-50 ${selectedId === r.id ? "bg-slate-50" : ""}`}
